...
class MenuItem {
  ...
  public:
	MenuItem(const eastl::string_view& name, psyqo::Rect posSizeRect) : m_name(name.data(), name.length()) {
		m_rect = posSizeRect;
	};

	MenuItem(const eastl::string_view& name, const eastl::string_view& text, psyqo::Rect posSizeRect)
		: MenuItem(name, posSizeRect) {
	...
	MenuItem(const eastl::string_view& name, const eastl::string_view& text, psyqo::Rect posSizeRect,
			 psyqo::Color defaultTextColour, psyqo::Color selectedTextColour)
		: MenuItem(name, text, posSizeRect) {
	...
	const constexpr bool IsEnabled() const { return m_isEnabled; }
	...
	void SetText(const eastl::string_view& text) { m_text.SetDisplayText(text); }
