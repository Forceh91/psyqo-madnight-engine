...
void Renderer::RenderGameObjects(uint32_t deltaTime, const psyqo::Matrix33& cameraRotationMatrix) {
    ...
    if (!gameObjects.empty()) {
        ...
        for (const auto& gameObject : gameObjects) {
            ...
            else if (mesh->type == 2) // triangles
            {
                for (int i = 0; i < mesh->facesCount; i++) {
                    // load the first 3 verts into the GTE. remember it can only handle 3 at a time
                    psyqo::GTE::writeSafe<psyqo::GTE::PseudoRegister::V0>(mesh->verticesOnBonePos[mesh->vertexIndices[i].i1]);
                    psyqo::GTE::writeSafe<psyqo::GTE::PseudoRegister::V1>(mesh->verticesOnBonePos[mesh->vertexIndices[i].i3]);
                    psyqo::GTE::writeSafe<psyqo::GTE::PseudoRegister::V2>(mesh->verticesOnBonePos[mesh->vertexIndices[i].i4]);
                    ...
                    // if its out of the screen space we can clip too
                    //if (quad_clip(&screen_space, &projected[0], &projected[1], &projected[2], &projected[3]))
                    if (tri_clip(&screen_space, &projected[0], &projected[1], &projected[2]))
                        continue;
                    ...
                    // set its colour, and make it opaque
                    tri.primitive.setColorA({ mesh->vertexColours[mesh->vertexIndices[i].i1].r, mesh->vertexColours[mesh->vertexIndices[i].i1].g, mesh->vertexColours[mesh->vertexIndices[i].i1].b });
                    tri.primitive.setColorB({ mesh->vertexColours[mesh->vertexIndices[i].i3].r, mesh->vertexColours[mesh->vertexIndices[i].i3].g, mesh->vertexColours[mesh->vertexIndices[i].i3].b });
                    tri.primitive.setColorC({ mesh->vertexColours[mesh->vertexIndices[i].i4].r, mesh->vertexColours[mesh->vertexIndices[i].i4].g, mesh->vertexColours[mesh->vertexIndices[i].i4].b });
                    ...
                    if (texture) {
                        ...
                        // set its uv coords
                        auto uvA = mesh->uvs[mesh->uvIndices[i].i3];
                        tri.primitive.uvA.u = offset.pos.x + uvA.u;
                        tri.primitive.uvA.v = offset.pos.y - uvA.v;

                        auto uvB = mesh->uvs[mesh->uvIndices[i].i1];
                        tri.primitive.uvB.u = offset.pos.x + uvB.u;
                        tri.primitive.uvB.v = offset.pos.y - uvB.v;

                        auto uvC = mesh->uvs[mesh->uvIndices[i].i4];
                        tri.primitive.uvC.u = offset.pos.x + uvC.u;
                        tri.primitive.uvC.v = offset.pos.y - uvC.v;
                    }
