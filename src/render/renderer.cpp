...
void Renderer::Render(void)
{
    ...
    for (auto &gameObject : gameObjects)
    {
        ...
        auto transform = psyqo::SoftMath::generateRotationMatrix33(gameObject->rotation().x, psyqo::SoftMath::Axis::X, g_madnightEngine.m_trig);
        auto rot = psyqo::SoftMath::generateRotationMatrix33(gameObject->rotation().y, psyqo::SoftMath::Axis::Y, g_madnightEngine.m_trig);

        // combine object and camera rotations
        psyqo::Matrix33 finalMatrix;
        psyqo::SoftMath::multiplyMatrix33(CameraManager::get_rotation_matrix(), rot, &finalMatrix);
