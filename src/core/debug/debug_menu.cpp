	...
	g_madnightEngine.m_input.setOnEvent([&](auto event) {
		...
		if (m_isEnabled) {
			...
			case 1: // debug hud/heap usage
				if (event.button == psyqo::AdvancedPad::Button::Left ||
					event.button == psyqo::AdvancedPad::Button::Right) {
			...
			if (event.button == psyqo::AdvancedPad::Button::L1 || event.button == psyqo::AdvancedPad::Button::L2 ||
				event.button == psyqo::AdvancedPad::Button::R1 || event.button == psyqo::AdvancedPad::Button::R2) {
				if (!m_startDebugMenuOpenCapture) {
					m_startDebugMenuOpenCapture = Renderer::Instance().GPU().getFrameCount();
				}

				if (event.button == psyqo::AdvancedPad::Button::L1) {
					m_debugMenuOpenCapturedInputs |= (1 << 0);
				}
				if (event.button == psyqo::AdvancedPad::Button::L2) {
					m_debugMenuOpenCapturedInputs |= (1 << 1);
				}
				if (event.button == psyqo::AdvancedPad::Button::R1) {
					m_debugMenuOpenCapturedInputs |= (1 << 2);
				}
				if (event.button == psyqo::AdvancedPad::Button::R2) {
					m_debugMenuOpenCapturedInputs |= (1 << 3);
				}
...
void DebugMenu::ToggleEnabled(void) { m_isEnabled = !m_isEnabled; }

void DebugMenu::Process(void) {
	// if we have a debug menu open capture frame and its been too long, stop
	if (m_startDebugMenuOpenCapture != 0 &&
		(Renderer::Instance().GPU().getFrameCount() - m_startDebugMenuOpenCapture) > 30) {
		ResetInputCapture();
	}
}
...
void DebugMenu::Draw(psyqo::GPU& gpu) {
	...
	auto font = Renderer::Instance().SystemFont();
	font->chainprintf(gpu, {.x = 3, .y = 3}, COLOUR_WHITE, "Debug Menu");
	font->chainprintf(gpu, {.x = 3, .y = 18}, m_selectedDebugOption == 0 ? COLOUR_YELLOW : COLOUR_WHITE,
					  "Raycast Distance:");
	font->chainprintf(gpu, {.x = 3, .y = 33}, m_selectedDebugOption == 0 ? COLOUR_YELLOW : COLOUR_WHITE, "   < %d >",
					  m_raycastDistance);
	font->chainprintf(gpu, {.x = 3, .y = 48}, m_selectedDebugOption == 1 ? COLOUR_YELLOW : COLOUR_WHITE, "Debug HUD:");
	font->chainprintf(gpu, {.x = 3, .y = 63}, m_selectedDebugOption == 1 ? COLOUR_YELLOW : COLOUR_WHITE, "   < %d >",
					  m_displayDebugHUD);
}
