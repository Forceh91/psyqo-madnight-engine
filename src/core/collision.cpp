...
bool Collision::IsSATCollision(const OBB &collisionA, const OBB &collisionB) {
  ...
  for (auto &axis : axesToCheck) {
    ...
    psyqo::Vec3 projectionA =
        GTEMath::ProjectVectorOntoAxes({collisionA.axes[0], collisionA.axes[1], collisionA.axes[2]}, normalizedAxis);
    psyqo::Vec3 projectionB =
        GTEMath::ProjectVectorOntoAxes({collisionB.axes[0], collisionB.axes[1], collisionB.axes[2]}, normalizedAxis);

    // now project the half extents
    auto aRadius = (projectionA.x.abs() * collisionA.halfExtents.x) + (projectionA.y.abs() * collisionA.halfExtents.y) +
                   (projectionA.z.abs() * collisionA.halfExtents.z);
    auto bRadius = (projectionB.x.abs() * collisionB.halfExtents.x) + (projectionB.y.abs() * collisionB.halfExtents.y) +
