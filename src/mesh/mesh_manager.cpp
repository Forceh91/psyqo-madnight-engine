...
psyqo::Coroutine<> MeshManager::LoadMesh(const eastl::string_view& meshName, MeshBin** meshOut) {
	...
	*meshOut = nullptr;
	...
	if (version > 1 && loaded_mesh.mesh.hasSkeleton) {
		...
		loaded_mesh.mesh.verticesOnBonePos =
			(psyqo::Vec3*)psyqo_malloc(sizeof(psyqo::Vec3) * loaded_mesh.mesh.vertexCount);
		...
		for (int32_t i = 0; i < loaded_mesh.mesh.numBones; i++) {
			...
			// local rotation
			__builtin_memcpy(&loaded_mesh.mesh.skeleton->bones[i].localRotation.w.value, ptr,
							 sizeof(int16_t)); // 2 bytes
			ptr += sizeof(int16_t);

			__builtin_memcpy(&loaded_mesh.mesh.skeleton->bones[i].localRotation.x.value, ptr,
							 sizeof(int16_t)); // 2 bytes
			ptr += sizeof(int16_t);

			__builtin_memcpy(&loaded_mesh.mesh.skeleton->bones[i].localRotation.y.value, ptr,
							 sizeof(int16_t)); // 2 bytes
			ptr += sizeof(int16_t);

			__builtin_memcpy(&loaded_mesh.mesh.skeleton->bones[i].localRotation.z.value, ptr,
							 sizeof(int16_t)); // 2 bytes
			ptr += sizeof(int16_t);
