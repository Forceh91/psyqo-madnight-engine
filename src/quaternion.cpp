...
void Quaternion::Normalize() {
	...
	auto r = psyqo::FixedPoint<>(1 / psyqo::SoftMath::squareRoot(psyqo::FixedPoint<>(s)));
	w *= r;
	x *= r;
	y *= r;
	z *= r;
...
psyqo::Matrix33 Quaternion::ToRotationMatrix() const {
	...
	auto one = psyqo::FixedPoint<>(1);
	auto two = psyqo::FixedPoint<>(2);
...
Quaternion FromEulerAngles(psyqo::Angle pitch, psyqo::Angle yaw, const psyqo::Trig<>& trig) {
	psyqo::FixedPoint<> cr = 1;
	psyqo::FixedPoint<> sr = 0;

	auto cosPitch = trig.cos(pitch / 2);
	auto sinPitch = trig.sin(pitch / 2);

	auto cosYaw = trig.cos(yaw / 2);
	auto sinYaw = trig.sin(yaw / 2);
...
Quaternion FromEulerAngles(psyqo::Angle pitch, psyqo::Angle yaw, psyqo::Angle roll, const psyqo::Trig<>& trig) {
	auto cr = trig.cos(roll / 2);
	auto sr = trig.sin(roll / 2);
	auto cp = trig.cos(pitch / 2);
	auto sp = trig.sin(pitch / 2);
	auto cy = trig.cos(yaw / 2);
	auto sy = trig.sin(yaw / 2);
...
Quaternion Slerp(const Quaternion& q1, const Quaternion& q2, psyqo::FixedPoint<> factor) {
	...
	auto q1Factor = psyqo::FixedPoint<>(1.0_fp - factor);
	auto q2Factor = psyqo::FixedPoint<>(factor);
...
Quaternion operator-(const Quaternion& q) {
	return Quaternion{q.w,	// w stays the same
