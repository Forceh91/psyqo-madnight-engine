...
- `GameObject`, `Billboard`, and `ParticleEmitter` are meant to be created through their manager's `Create*` (`GameObjectManager`, `BillboardManager`, `ParticleEmitterManager`), and `Particle` by its `ParticleEmitter`. Their default constructors are still `public`, so the compiler won't stop you constructing one directly.
