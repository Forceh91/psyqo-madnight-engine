...
- `GameObject`, `Billboard`, `Particle`, and `ParticleEmitter` constructors are now `private`, with their owning manager (`GameObjectManager`, `BillboardManager`, `ParticleEmitterManager`) as a `friend` — they were already only meant to be constructed via `Create*`, this just makes it compiler-enforced.
